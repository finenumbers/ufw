...
Ogni release pubblica **`latest`** più tag di versione (es. `v0.6.1`, `0.6.1`). I deploy di produzione usano **`latest`** per impostazione predefinita — nessuna versione richiesta in `.env`.
