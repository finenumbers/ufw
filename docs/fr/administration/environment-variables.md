...
Depuis **v0.5.1**, les variables héritées telles que `PORT_SCAN_RATE_LIMIT_WINDOW_MS`, `DOCKER_REFRESH_RATE_LIMIT_WINDOW_MS` et `DOCKER_CONTROL_RATE_LIMIT_WINDOW_MS` sont **ignorées** si elles restent dans `.env`.
