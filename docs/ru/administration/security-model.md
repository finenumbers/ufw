...
- Скан только по `Server.host` из базы данных
- Naabu + Nmap внутри `ufw-app` (connect-scan, без произвольных целей)
- Rate limit на сервер; события аудита
- Нужен **исходящий доступ** контейнера к managed-хостам на сканируемых портах — см. [Сканирование портов](../deployment/port-scan.md)

## Мониторинг Docker (опционально)

При `DOCKER_MONITOR_ENABLED=true`:

- Inventory и управление только через **SSH** на зарегистрированных серверах
- Валидация container ID/name; только действия `START` / `STOP` / `RESTART`
- Rate limits и audit на refresh и control
- SSH-пользователю нужен доступ к Docker CLI — см. [Мониторинг Docker](../deployment/docker-monitor.md)
