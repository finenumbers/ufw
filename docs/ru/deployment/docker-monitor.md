...
Пока обновление инвентаря выполняется, интерфейс опрашивает лёгкий status endpoint. Polling **сразу**, затем каждую **1 с** пока операция активна (backoff после ~30 мин). При завершении в banner панели обновляются немедленно. Banner опрашивает API каждую **1 с** в статусе RUNNING.
